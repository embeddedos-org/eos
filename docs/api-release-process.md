...
├── release/vX.Y        ← Release candidates (branched from develop)
└── hotfix/vX.Y.Z       ← Emergency fixes (branched from release tag)
```

| Branch | Purpose | Merges From | Merges To |
|--------|---------|-------------|-----------|
| `main` | Stable, tagged releases only | `release/vX.Y`, `hotfix/vX.Y.Z` | — |
| `develop` | Integration branch for next release | Feature branches | `release/vX.Y` |
| `release/vX.Y` | Release candidates, final stabilization | `develop` | `main` |
| `hotfix/vX.Y.Z` | Emergency fixes for released versions | `main` (tagged) | `main` + `develop` |

### Release Checklist

Before any release tag is created, **all** of the following must be satisfied:

- [ ] All CI checks pass (build, test, lint, static analysis) on all 3 platforms
- [ ] Test coverage ≥ 50% (line coverage across all modules)
- [ ] `CHANGELOG.md` updated with all changes since last release
- [ ] Version bumped in `CMakeLists.txt` (`project(eos VERSION X.Y.Z)`)
- [ ] Traceability matrix (`docs/compliance/traceability_matrix.md`) is current
- [ ] Safety documentation reviewed (if applicable to this release)
- [ ] API documentation generates with 0 Doxygen warnings
- [ ] No Critical or High CVEs in `scripts/cve_check.sh` report
- [ ] SBOM regenerated for this release
- [ ] Release branch passes all fuzz test harnesses

### Hotfix Process

Hotfixes are used for critical bug fixes or security patches on released versions:
...
# or develop the fix directly on the hotfix branch

# 3. Update version and changelog
# Bump PATCH version in CMakeLists.txt: 1.2.0 → 1.2.1
...
- Hotfix branches contain **only** the fix and version bump — no new features.
- The fix must pass all CI checks before merging to `main`.
- After tagging, the hotfix branch is deleted.
