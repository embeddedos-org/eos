...
/* eos_pkg_set_trust_anchor() / eos_pkg_trust_anchor() now live in
 * eos_pkg_trust_anchor.c -- pure in-memory state with no POSIX dependency,
 * split out so it can be linked into a cross-target build independently of
 * the directory-listing and process-spawning code below. See that file and
 * #133. */
