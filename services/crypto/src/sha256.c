...
#define SIG0_256(x) (RR32(x,7)^RR32(x,18)^((x)>>3))
#define SIG1_256(x) (RR32(x,17)^RR32(x,19)^((x)>>10))
...
static void sha256_transform(EosSha256 *ctx, const uint8_t *block) {
    ...
    for (int i=0;i<64;i++){
        t1=h+EP1_256(e)+CH(e,f,g)+K256[i]+w[i];
