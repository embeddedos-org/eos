...
| T-009 | Zero-fill newly exposed RAM-filesystem ranges | backend | independent reviewer | Warning-clean focused build and direct filesystem run: **15/15 passed**; the same tests abort on the stale-byte assertion against the old implementation. Generic/RTOS C build passed and Python suites reported **10 passed**. Host CTest launch is unverified because the Windows cross-build emits extensionless PE test paths. The separate write-length overflow is already tracked by PR #95. |
