...
| T-009 | Restore kernel build broken by commit `02c490a` (PR #55) | backend | — | `kernel/src/sync.c` referenced `m->original_prio` (field absent from `mtx_t`), `task_valid`, `g_blocked_on`, `pi_propagate` (all removed) — 9 compile errors; `kernel/src/task.c` had a duplicate `eos_task_set_current_internal` — 1 error. Restored the transitive priority-inheritance machinery from `9a61678`, removed the duplicate definition, re-registered the two kernel tests the commit dropped (`test_task_stats`, `test_tick_overflow`), and repaired the clobbered CHANGELOG. Verified with `zig cc`: `sync.c`/`task.c` compile clean under `-Wall -Wextra`; `test_priority_inherit` 6/6 and `test_kernel` 12/12 pass. |
