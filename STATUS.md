...
| HAL — extended peripherals (host) | (in `libeos_hal.a`) | Experimental | `hal_extended_stubs.c` is 707 lines and contains 268 `(void)param;` casts; ADC, I²C, SPI and most other extended peripherals remain stub-only. PWM now has a real host backend (`hal_pwm.c`) — a state-recording stub, not a waveform generator — with `test_pwm` **28/28 pass**. |
| HAL — extended peripherals (STM32F4) | (in `libeos_hal.a`, cross build) | **Planned** | PWM and ADC each have STM32F4 register-level code (`hal_pwm.c`, `hal_extended.c`), but neither is compiled by any build target in this repository: the ARM CI job (`.github/workflows/build.yml`) compiles an explicit source list that omits both files, and the CMake cross-build (`toolchains/arm-cortex-m4.cmake`) sets `EOS_MCU_STM32F4` as a cache variable that is never converted into a compiler `-D` flag either. No test exercises this code, and no ADC test exists at all. |
