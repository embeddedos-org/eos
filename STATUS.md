...
| HAL — extended peripherals (ADC, PWM, I²C, SPI, …) | (in `libeos_hal.a`) | Experimental | `hal_extended_stubs.c` is 740 of the HAL's 3,185 lines and contains 144 `(void)param;` casts. The API exists; many peripherals do not. PWM now has platform-specific implementations for STM32F4 and host builds; `test_pwm` **25/25 pass**. ADC has STM32F4 implementation and passes basic tests. |
